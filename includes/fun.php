...
function content_fields($id,$v_arr='',$edit='false'){
	...
	if(!empty($field)){
	   unset($field[0]);
		$field['field_value']=trim($field['field_value']);
...
function ddchanel($id="2"){
if(empty($id)){$id="2";}
$sql="select * from ".DB_PRE."maintb where id=".$id;
$arc_rel=$GLOBALS['mysql']->fetch_asc($sql);
$arc_title=$arc_rel[0]['channel'];
//echo $sql;
return $arc_title;
}
//表单
...
function html($f_name,$f_value){
	...
	$str="<textarea name=\"fields[{$f_name}]\" id=\"{$f_name}\" {$s} style='width:800px;height:400px;visibility:hidden;'>{$f_value}</textarea>";
	 return $str;
...
function select($f_name,$f_value,$e_value=""){
   // echo $f_value;
	...
	$value=preg_split('/\n/',$f_value);
	if($f_name=="shengfeng"){
	$str="<select id=\"{$f_name}\" name=\"fields[{$f_name}]\"  onChange=\"changeComplexProvince(this.value, sub_array, 'shi', 'quxian');\">";
	} else if($f_name=="shi"){
	$str="<select id=\"{$f_name}\" name=\"fields[{$f_name}]\" onChange=\"changeCity(this.value,'quxian','quxian');\">";
	}else if($f_name=="quxian"){
...
function page($url='',$page,$query='',$totalnum,$totalpages,$cate_id='',$ishtml=0){
	...
	$shuzhu['shouye']=$url.'?page=1'.$query;
	$shuzhu['pre_page']=$url."?page=".$pre_page.$query;
	$shuzhu['next_page']=$url."?page=".$next_page.$query;
	$shuzhu['totalpages']=$url."?page=".$totalpages.$query;
	$shuzhu['str']=$str;
	//echo $str;
	return $str;
}
...
//列表页分页函数
function page3($url='',$page,$query='',$totalnum,$totalpages,$cate_id='',$ishtml=0){
	global $language,$_confing,$php_file_copy;
	$str="";
	$op="";
	$pre_page=($page-1<0)?1:($page-1);
	...
	if($_confing['web_html'][0]&&!$ishtml){
		...
		//动态分页
	/*	$str.=($page>1)?'<a class="off" href="'.$url.'?page=1'.$query.'">'.$language['pagehome'].'</a>':'<span class="off">'.$language['pagehome'].'</span>';
		$str.=($page>1)?'<a class="off" href="'.$url.'?page='.$pre_page.$query.'">'.$language['pagapre'].'</a>':'<span class="off">'.$language['pagapre'].'</span>';
		*/
		if($totalpages>$page_count){
			if($page<=$offset){
				$page_star=1;
		...
		$str.="<li class=\"fenkong\">";
		for($i=$page_star;$i<=$page_end;$i++){
			$class=($page==$i)?"fenyeon":"";
			if($i==$page_star&&$i!=1){$str.='...';}
			$str.='<span class=focus2><a class="'.$class.'" href="'.$url.'?page='.$i.$query.'">'.$i.'</a></span>';
			if($i==$page_end&&$i!=$totalpages){$str.='...';}
		}
		$str.='</li>';
		for($i=1;$i<=$totalpages;$i++){
...
//		$str.=($page<$totalpages)?'<a class="off" href="'.$url.'?page='.$next_page.$query.'">'.$language['pagenext'].'</a>':'<span class="off">'.$language['pagenext'].'</span>';
//		$str.=($page<$totalpages)?'<a class="off" href="'.$url.'?page='.$totalpages.$query.'">'.$language['pageend'].'</a>':'<span class="off">'.$language['pageend'].'</span>';
	}
	$shuzhu['shouye']=$url.'?page=1'.$query;
	$shuzhu['pre_page']=$url."?page=".$pre_page.$query;
	$shuzhu['next_page']=$url."?page=".$next_page.$query;
...
//列表页分页函数
function page4($url='',$page,$query='',$totalnum,$totalpages,$cate_id='',$ishtml=0){
	global $language,$_confing,$php_file_copy;
	$str="";
	$op="";
	$pre_page=($page-1<0)?1:($page-1);
...
//列表页分页函数
function page2($url='',$page,$query='',$totalnum,$totalpages,$cate_id='',$ishtml=0){
	global $language,$_confing,$php_file_copy;
	$str="";
	$op="";
	$pre_page=($page-1<0)?1:($page-1);
...
function is_member_login(){
	if($_SESSION['member_login']&&$_SESSION['member_user']){
		if(time()-$_SESSION['m_time']>3600){
			login_out();
		}else{
			$_SESSION['m_time']=time();
			@session_regenerate_id();
		}
		return 1;
	}else{
...
function member_info($value='')
{
	$member=$GLOBALS['mysql'] -> fetch_asc("select * from ".DB_PRE."member where id=".$_SESSION['member_id']);
...
function get_cate_list($cate,$parent,$haschild,$level=0){
		...
		$level=$level+1;
			foreach($cate as $k=>$v){
				...
				echo "<div id=\"border\" style=\"border-bottom:1px dashed #ccc; padding:2px 0;height:25px; line-height:25px;\" onMouseOut=\"this.style.background='#FFFFFF'\" onMouseOver=\"this.style.background='#68D9F7'\">
					<div class=\"left\" id=\"show\" style=\"padding-left:".$pd."px; cursor:pointer\"><span class=\"exp\" onclick=\"show_list(this);\">&nbsp;</span>
					<span class=\"cata\"><a style='display: inline-block; width: 140px;' href=\"".CMS_SELF.$list_php."?id={$v['id']}\" target=\"_blank\">".$is_cate_type.$v['cate_name']."</a>
					<div style='display: inline-block; width: 300px;'>(<span style=\"color:#999\">排序</span><em rel=\"order\" style=\"font-style:normal; padding:0 8px;\" id=\"order_num\"><span id=\"".$v['id']."\">".$v['cate_order']."</span></em>&nbsp;
					<span style=\"color:#999\">栏目id</span>:".$v['id']."&nbsp;<span style=\"color:#999\">模板标示ID:</span><em rel=\"tpl\" style=\"font-style:normal; padding:0 8px;\" id=\"order_num\"><span id=\"".$v['id']."\">".$v['temp_id']."</span></em>".$is_gd.")</div>";
				$cate_nav=empty($v['cate_nav'])?array(''):explode(',',$v['cate_nav']);
				echo empty($cate_nav[0])?"<span style='color:#3366FF;display: inline-block; width: 100px;'></span>":"";
				echo in_array('2',$cate_nav)?"<span style=\"color:#3366FF;display: inline-block; width: 100px;'\">导航中部显示</span>":"";
				echo in_array('3',$cate_nav)?"<span style=\"color:#FFCC66;display: inline-block; width: 100px;'\">导航底部显示</span>":"";
				
				if($v['cate_hide']){
					echo "<span style=\"color:red; padding:0 3px;\">隐藏</span>";
				}
				echo "<div style='display: inline-block; width: 200px;'>列表页:<font color='red'>".$v['cate_tpl_list']."</font></div>内容页:<font color='red'>".$v['cate_tpl_content']."</font>";
...
//获得下级栏目
function get_child_cate($parent_id,$lang,$is_content=0,$cateid='',$yangshi='focus'){
	...
	foreach($rel as $row){
		if(!$row['cate_hide']){
			...
			$child[$row['id']]['cate_pic1']=empty($row['cate_pic1'])?CMS_URL.'upload/no_pc.gif':CMS_URL.'upload/'.$row['cate_pic1'];
			$child[$row['id']]['cate_pic2']=empty($row['cate_pic2'])?CMS_URL.'upload/no_pc.gif':CMS_URL.'upload/'.$row['cate_pic2'];
			$child[$row['id']]['cate_pic3']=empty($row['cate_pic3'])?CMS_URL.'upload/no_pc.gif':CMS_URL.'upload/'.$row['cate_pic3'];
...
function get_post_brandlist()
{
	$sql="select * from ".DB_PRE."brand";
	$rel=$GLOBALS['mysql']->fetch_asc($sql);
	return $rel;
}
