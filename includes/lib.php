 ...
 * @return  array
 */
function list_article($order='',$pmfs='top',$pmfs2='id',$pmid1="f",$pmid2="f"){
	global $page,$cat_id,$list_cate,$r_count,$page_size,$child,$lang,$cate_info,$channel_info,$category,$brand;
	...
	$offset = ($page-1) * $row;
	$rel = $GLOBALS['mysql'] -> fetch_asc("select m.*,f.* from ".DB_PRE."maintb as m left join ".DB_PRE.$table." as f on m.id=f.id where m.lang='".$lang."' and m.id=f.id and category in (".$list_cate.") order by ".$pmid1.".".$pmfs." ".$order.",".$pmid2.".".$pmfs2." ".$order." limit ".$offset.",".$page_size);
	$path = CMS_URL;
	if(!empty($rel))
	{
		$i = 1;
 ...
 * 根据品牌id  获取该id下的所有品牌
 * */
function list_brand($order='',$pmfs='top',$pmfs2='id',$pmid1="f",$pmid2="f"){
	global $page,$cat_id,$list_cate,$r_count,$page_size,$child,$lang,$cate_info,$channel_info,$category,$brand;
	$_confing=get_confing($lang);
	if(empty($cat_id) || empty($list_cate))
	{
	...
	$page = empty($page) ? 1 : $page;
	$offset = ($page-1) * $row;
	$rel = $GLOBALS['mysql'] -> fetch_asc("select m.*,f.* from ".DB_PRE."maintb as m left join ".DB_PRE.$table." as f on m.id=f.id where brand=".$brand." and m.lang='".$lang."' and m.id=f.id  order by ".$pmid1.".".$pmfs." ".$order.",".$pmid2.".".$pmfs2." ".$order." limit ".$offset.",".$page_size);
	$path = CMS_URL;
	if(!empty($rel))
	{
		$i = 1;
...
function list_article_by_id($id1="",$order='',$pmfs='top',$pmfs2='id',$pmid1="f",$pmid2="f"){
	...
	$rel = $GLOBALS['mysql'] -> fetch_asc("select m.*,f.* from ".DB_PRE."maintb as m left join ".DB_PRE.$table." as f on m.id=f.id where m.category=".$id1." and  m.lang='".$lang."' and m.id=f.id  order by ".$pmid1.".".$pmfs." ".$order.",".$pmid2.".".$pmfs2." ".$order." limit ".$offset.",30");
	$child=$GLOBALS['mysql'] -> fetch_asc("select * from ".DB_PRE."category where cate_parent =".$id1);
	$str_child="";
	foreach($child as $k=>$v){
		$str_child=$str_child.$v["id"].",";
	}
	$str_child = substr($str_child,0,strlen($str_child)-1);
	if($str_child!=""){
		$rel1 = $GLOBALS['mysql'] -> fetch_asc("select m.*,f.* from ".DB_PRE."maintb as m left join ".DB_PRE.$table." as f on m.id=f.id where m.category in (".$str_child.") and  m.lang='".$lang."' and m.id=f.id  order by ".$pmid1.".".$pmfs." ".$order.",".$pmid2.".".$pmfs2." ".$order);
		foreach($rel1 as $k =>$v){
			array_push($rel,$v);
		}
	}
	...
	if(!empty($rel))
	{
		...
		foreach($rel as $k=>$v){
			...
			$rel[$k]['first']=($i==1)?1:0;
			$rel[$k]['last']=($num==$i)?1:0;
			$rel1 = $GLOBALS['mysql'] -> fetch_asc("select * from ".DB_PRE."brand  where id=".$rel[$k]['brand']);
			$rel[$k]['brandName']=$rel1[0]["name"];
			$i=$i+1;
		}
	}
	return $rel;
...
获得列表内容页的最后一页数值
*/
function list_likearticleend($nrid='',$ys=''){
if(empty($nrid)){
return;
}
if(empty($ys)){
...
$cxlb="select *  from ".DB_PRE."maintb where id=".$nrid;
$cxlbnr=$GLOBALS['mysql'] -> fetch_asc($cxlb);
$category=$cxlbnr[0]['category'];
$endnum=0;
if(!empty($category)){
$cxzs="select count(*) as zs from ".DB_PRE."maintb where category=".$category;
$cxzsq=$GLOBALS['mysql'] -> fetch_asc($cxzs);
$zs=$cxzsq[0]['zs'];
$endnum=ceil($zs/$ys);
}
return $endnum;
 ...
 * @return  array
 */
function list_likearticle($order='',$nrid='',$fenye='',$ys=''){
	global $page,$cat_id,$list_cate,$r_count,$page_size,$child,$lang,$cate_info,$channel_info,$category;
	$_confing=get_confing($lang);
	if(empty($fenye)){
	$page="1";
	...
	$page = empty($page) ? 1 : $page;
	$offset = ($page-1) * $row;
	$rel = $GLOBALS['mysql'] -> fetch_asc("select * from ".DB_PRE."maintb as m  where m.lang='".$lang."' and category = ".$list_cate." order by m.top ".$order.",m.id ".$order." limit ".$offset.",".$page_size);
	$path = CMS_URL;
	if(!empty($rel))
	{
		$i = 1;
...
调用栏目标题
*/
function get_cate_newname($lmid,$dynr){
	global $channel_info,$cate_info,$category,$lang;
	$cate=empty($lmid)?$GLOBALS['cat_id']:$lmid;
	$sql="select * from ".DB_PRE."category where lang='".$GLOBALS['lang']."' and id='".$cate."'";
	$t_id=$GLOBALS['mysql']->fetch_asc($sql);
	if($dynr=="")
	{
		$chanel=$t_id[0]['cate_name'];
		return $chanel;
	}
	if($dynr=="lmmc")
	{
	...
	if($dynr=="lmnr")
	{
		$chanel=$t_id[0]['cate_content'];
		return $chanel;
	}
	if($dynr=="tp")
	{
		$chanel=CMS_URL.'upload/'.$t_id[0]['cate_pic1'];
		return $chanel;
	}
...
//调用一级栏目的标题简介和图片
function get_cate_newname2($lmid,$dynr){
global $channel_info,$cate_info,$category,$lang;
$cate=empty($lmid)?$GLOBALS['cat_id']:$lmid;
$dynr=empty($dynr)?"lmmc":$dynr;
$catep=$cate;
$idss=1;
while($catep!=0 && $idss<=5){
$cate=$catep;
$sql="select cate_parent from ".DB_PRE."category where lang='".$GLOBALS['lang']."' and id='".$cate."'";
$t_id=$GLOBALS['mysql']->fetch_asc($sql);
$catep=$t_id[0]['cate_parent'];
$idss=$idss+1;
}
if($catep==0){
$sql2="select * from ".DB_PRE."category where lang='".$GLOBALS['lang']."' and id='".$cate."'";
$t_id2=$GLOBALS['mysql']->fetch_asc($sql2);
if($dynr=="lmmc")
{
$nr=$t_id2[0]['cate_name'];
}
if($dynr=="lmnr"){
$nr=$t_id2[0]['cate_content'];
}
if($dynr=="tp"){
$nr=CMS_URL.'upload/'.$t_id2[0]['cate_pic1'];
}
}
return $nr;
}

function get_cate_img($num)
{
	global $channel_info,$cate_info,$category,$lang;
	$cate=$GLOBALS['cat_id'];
	$num=empty($num)?"1":$num;
	$sql="select * from ".DB_PRE."category where lang='".$GLOBALS['lang']."' and id='".$cate."'";
	$result=$GLOBALS['mysql']->fetch_asc($sql);
	return CMS_URL.'upload/'.$result[0]['cate_pic'.$num];
}


//调用一级栏目的标题简介和图片
function get_cate_newname3($lmid,$dynr){
global $channel_info,$cate_info,$category,$lang;
$cate=empty($lmid)?$GLOBALS['cat_id']:$lmid;
$dynr=empty($dynr)?"lmmc":$dynr;
$catep=$cate;
...
//echo $idss;
if($idss>1){
$sql2="select count(*) as zs from ".DB_PRE."category where lang='".$GLOBALS['lang']."' and cate_parent='".$cate."'";
$sql23=$GLOBALS['mysql']->fetch_asc($sql2);
$zs=$sql23[0]['zs'];
//echo $zs;
if($zs==0 && $catep!=0){
$cate=$catep;
...
*在任意页面通过模板id调用栏目
*/
function get_indexlist_nav($tpl_id='',$yangshi=''){
	global $channel_info,$cate_info,$category,$lang;
	//获取后台配置信息
	$tpl_arr=array();
	if(empty($yangshi)){
	...
	if(!empty($tpl_id)){
		$sql="select * from ".DB_PRE."category where lang='".$GLOBALS['lang']."' and temp_id='".$tpl_id."'";
		$t_id=$GLOBALS['mysql']->fetch_asc($sql);;
		$lanmuid=$t_id[0]['id'];
		$chanel=$t_id[0]['cate_channel'];
		$sql2="select * from ".DB_PRE."channel where id='".$chanel."'";
		$cid=$GLOBALS['mysql']->fetch_asc($sql2);
		$lxmc=$cid[0]['channel_mark'];
	}
	
	
	$cate=empty($tpl_id)?$GLOBALS['cat_id']:$lanmuid;
	$catep=$cate;
	$idss=1;
	while($catep!=0 && $idss<=5){
		$cate=$catep;
		$sql23="select cate_parent from ".DB_PRE."category where lang='".$GLOBALS['lang']."' and id='".$cate."'";
		$t_id23=$GLOBALS['mysql']->fetch_asc($sql23);
		$catep=$t_id23[0]['cate_parent'];
		$idss=$idss+1;
	}
	//$cate=empty($cate)?0:$cate;//存在父栏目输出下级，不存在输出父栏目
	//如果当前栏目没有下级，存在上级则返回上级
	...
	$list_php=empty($channel_info['list_php'])?$lxmc.'/'.$lxmc.'.php':$channel_info['list_php'];
	$content_php=empty($channel_info['content_php'])?'show_content.php':$channel_info['content_php'];	
	if(file_exists(DATA_PATH.'cache_cate/cate_list_'.$lang.'.php')){include(DATA_PATH.'cache_cate/cate_list_'.$lang.'.php');}//当前语言下的栏目
	...
	if(!empty($cate_list)){
			foreach($cate_list as $k=>$v){
				//$list_php=empty($channel_info['list_php'])?$lxmc3.'/'.$lxmc3.'.php':$channel_info['list_php'];
				if($v['cate_parent']==$cate&&!$v['cate_hide']){
				$chenlmz=$v['cate_channel'];
				$sql3="select * from ".DB_PRE."channel where id='".$chenlmz."'";
		        $cid3=$GLOBALS['mysql']->fetch_asc($sql3);
		        $lxmc3=$cid3[0]["list_php"];
				$list_php=$lxmc3;
				$path=CMS_URL;
				$nav_arr[$k]['target']=intval($v['cate_is_open'])?'target="_blank"':'';//新窗口
				if($v['cate_tpl']==2){
				...
				$nav_arr[$k]['cate_content']=$cate_child_info['cate_content'];
				$nav_arr[$k]['cate_name']=$v['cate_name'];
				$nav_arr[$k]['cate_order']=$v['cate_order'];
				$nav_arr[$k]['id']=$v['id'];
				$nav_arr[$k]['child']=get_child_cate($v['id'],$GLOBALS['lang'],$GLOBALS['cat_id'],"","lefttwonow");
				}
			}
		}
		if(!empty($nav_arr)){
...
*在任意页面通过模板id调用栏目
*/
function get_indexlist_nav2($tpl_id='',$yangshi=''){
	global $channel_info,$cate_info,$category,$lang;
	//获取后台配置信息
	$tpl_arr=array();
	if(empty($yangshi)){
 ...
 * @return  string
 */
function list_page($biaozhi=''){
	global $list_php,$page,$cat_id,$r_count,$page_size,$page_count,$ishtml;
	//print_r(page('',intval($page),'&id='.$cat_id,$r_count,$page_count,$cat_id,$ishtml));
	$shuzhu=page('',intval($page),'&id='.$cat_id,$r_count,$page_count,$cat_id,$ishtml);
	
	return $shuzhu;
	
}

function list_page3($biaozhi=''){
	global $list_php,$page,$cat_id,$r_count,$page_size,$page_count,$ishtml;
	//print_r(page('',intval($page),'&id='.$cat_id,$r_count,$page_count,$cat_id,$ishtml));
	$shuzhu=page3('',intval($page),'&id='.$cat_id,$r_count,$page_count,$cat_id,$ishtml);
	
	return $shuzhu;
	
}

function list_page4($biaozhi='',$lmid=''){
	global $list_php,$page,$cat_id,$r_count,$page_size,$page_count,$ishtml;
	//print_r(page('',intval($page),'&id='.$cat_id,$r_count,$page_count,$cat_id,$ishtml));
	if(empty($lmid)){
	$query="&lmid=83&id=";
	...
	$shuzhu=page4('',intval($page),$query.$cat_id,$r_count,$page_count,$cat_id,$ishtml);
	
	return $shuzhu;
	
}

function list_page2($biaozhi=''){
	global $list_php,$page,$cat_id,$r_count,$page_size,$page_count,$ishtml;
	//print_r(page('',intval($page),'&id='.$cat_id,$r_count,$page_count,$cat_id,$ishtml));
	$shuzhu=page2('',intval($page),'&id='.$cat_id,$r_count,$page_count,$cat_id,$ishtml);
	if(empty($biaozhi))
	{
	$biaozhi="str";
	}
...
//中间导航
function nav_middle($lxysmz=''){
	global $lang,$cat_id,$cateid,$parent_id;
	if(empty($lxysmz)){$lxysmz="focus";}
	if(file_exists(DATA_PATH.'cache_cate/cate_list_'.$lang.'.php')){include(DATA_PATH.'cache_cate/cate_list_'.$lang.'.php');}//当前语言下的栏目
	$_confing=get_confing($GLOBALS['lang']);
	$rel=array();
	...
	if(!empty($cate_list)){
		...
		if(in_array('2',$cate_nav)){
			...
			$rel[$k]['class']='';
			if($parent_id==$v['id']){$rel[$k]['class']=$lxysmz;}
			$rel[$k]['url']=$url;
...
//搜索
function get_search($yemashu=''){
global $channel,$category;
if(empty($yemashu)){
$yemashu=$GLOBALS['pagesize'];
}
$chanels=array("article","product","alone");
$sql1="select ".DB_PRE."maintb.*,".DB_PRE.$chanels[0].".content from  ".DB_PRE."maintb,".DB_PRE.$chanels[0]." where ".DB_PRE."maintb.id=".DB_PRE.$chanels[0].".id and (".DB_PRE."maintb.title like '%".$GLOBALS['key']."%' or ".DB_PRE.$chanels[0].".content like '%".$GLOBALS['key']."%') union all ";

$sql2="select ".DB_PRE."maintb.*,".DB_PRE.$chanels[1].".content from  ".DB_PRE."maintb,".DB_PRE.$chanels[1]." where ".DB_PRE."maintb.id=".DB_PRE.$chanels[1].".id and (".DB_PRE."maintb.title like '%".$GLOBALS['key']."%' or ".DB_PRE.$chanels[1].".content like '%".$GLOBALS['key']."%') union all ";

$sql3="select ".DB_PRE."maintb.*,".DB_PRE.$chanels[2].".content from  ".DB_PRE."maintb,".DB_PRE.$chanels[2]." where ".DB_PRE."maintb.id=".DB_PRE.$chanels[2].".id and (".DB_PRE."maintb.title like '%".$GLOBALS['key']."%' or ".DB_PRE.$chanels[2].".content like '%".$GLOBALS['key']."%')";

$sql4=$sql1.$sql2.$sql3;

$sql5=$sql4." order by id desc limit ".$GLOBALS['pagenum'].",".$yemashu;

if(!empty($GLOBALS['key'])){
$rel_search=$GLOBALS['mysql']->fetch_asc($sql5);
$rel=array();
$path=CMS_URL;
if(!empty($rel_search)){
$iii=1;
...
//搜索产品
function get_search_product($yemashu=''){
	global $channel,$category;
	if(empty($yemashu)){
		$yemashu=$GLOBALS['pagesize'];
	}
	...
	$sql2="select ".DB_PRE."maintb.*,".DB_PRE.$chanels[1].".content from  ".DB_PRE."maintb,".DB_PRE.$chanels[1]." where ".DB_PRE."maintb.id=".DB_PRE.$chanels[1].".id and (".DB_PRE."maintb.title like '%".$GLOBALS['key']."%' or ".DB_PRE.$chanels[1].".content like '%".$GLOBALS['key']."%')";
	...
	if(!empty($GLOBALS['key'])){
		$rel_search=$GLOBALS['mysql']->fetch_asc($sql2);
		$rel=array();
		$path=CMS_URL;
		if(!empty($rel_search)){
			$iii=1;
...
//搜索分店
function get_searchmendian($yemashu='',$sheng='',$shi='',$quxian=''){
global $channel,$category;
if(empty($yemashu)){
$yemashu=$GLOBALS['pagesize'];
}
if(empty($sheng) && empty($shi) && empty($quxian)){
$sheng2="33";
$shi2="3302";
$quxian2="330204";
$cxcs='select*from '.DB_PRE."maintb,".DB_PRE."mendian where lang='".$GLOBALS['lang']."' and ".DB_PRE."maintb.id=".DB_PRE."mendian.id  and shengfeng=".$sheng2." and shi=".$shi2." and quxian=".$quxian2." order by ".DB_PRE."maintb.id desc limit ".$GLOBALS['pagenum']."".$yemashu;
}
if(!empty($sheng)&& empty($shi) && empty($quxian)){
$sheng2=$sheng;
$cxcs='select*from '.DB_PRE."maintb,".DB_PRE."mendian where lang='".$GLOBALS['lang']."' and ".DB_PRE."maintb.id=".DB_PRE."mendian.id  and shengfeng=".$sheng2." order by ".DB_PRE."maintb.id desc limit ".$GLOBALS['pagenum']."".$yemashu;

}
if(!empty($sheng)&& !empty($shi) && empty($quxian)){
$sheng2=$sheng;
$shi2=$shi;
$cxcs='select*from '.DB_PRE."maintb,".DB_PRE."mendian where lang='".$GLOBALS['lang']."' and ".DB_PRE."maintb.id=".DB_PRE."mendian.id and shi=".$shi2." order by ".DB_PRE."maintb.id desc limit ".$GLOBALS['pagenum']."".$yemashu;
}
if(!empty($sheng) && !empty($shi) && !empty($quxian)){
$sheng2=$sheng;
$shi2=$shi;
...
if(true){
$rel_search=$GLOBALS['mysql']->fetch_asc($cxcs);
$rel=array();
$path=CMS_URL;
if(!empty($rel_search)){
$iii=1;
...
function get_search_page(){
	...
	$shuzhu=page('',$page,$query,$total_num,$total_page,'',1);
	return $shuzhu;
}
//搜索列表分页
function get_search_page2($biaozhi=''){
	global $page,$query,$total_num,$total_page;
	//return page('',$page,$query,$total_num,$total_page,'',1);
	$shuzhu=page2('',$page,$query,$total_num,$total_page,'',1);
	if(empty($biaozhi))
	{
	$biaozhi="str";
	}
	return $shuzhu[$biaozhi];
}
function get_search_page3(){
	global $page,$query,$total_num,$total_page;
	//return page('',$page,$query,$total_num,$total_page,'',1);
	$shuzhu=page3('',$page,$query,$total_num,$total_page,'',1);
	return $shuzhu;
}
//网站地图
 ...
 * @return  array
 */
 function get_tpl_cate_content($tpl_id='',$limit='0,5',$order_type='id',$filter='',$pic='no',$order='desc',$lang='',$cates=''){
 	$tpl_id = empty($tpl_id)?'':intval($tpl_id);//获得栏目模板ID，不存在获取内容页的栏目模板ID
	$lang=empty($lang)?$GLOBALS['lang']:$lang;
	if(empty($tpl_id)){return;}
	if(empty($order_type)){$order_type="id";}
	...
	$sql.=" and m.lang='".$lang."'";
	$sql.=(in_array($order_type,array('id','hits','addtime','updatetime')))?" GROUP BY m.id order by m.top desc, m.".$order_type:" GROUP BY  m.id order by m.".$order_type." ".$order.", f.paixu ";
	$sql.=(in_array($order,array('asc','desc')))?' '.$order:' desc';
	$sql.=empty($limit)?' limit 0,5':" limit ".$limit;
	...
	if(!empty($rel)){
		...
		foreach($rel as $k=>$v){
			...
			$rel[$k]['autoindex']=$i;
			$rel[$k]['nrid']=$v['id'];
			$rel[$k]['dqsj']=explode("-",$v['hdjssj']);
			$rel[$k]['dqsjn']=$rel[$k]['dqsj'][0];
			$rel[$k]['dqsjy']=$rel[$k]['dqsj'][1];
			$rel[$k]['dqsjr']=$rel[$k]['dqsj'][2];
			$rel[$k]['sfcx']=trim($v['sfcx']);
			$rel[$k]['sfdj']=trim($v['sfdj']);
			$rel[$k]['sfjk']=trim($v['sfjk']);
			$rel[$k]['first']=($i==1)?1:0;
			$rel[$k]['last']=($num==$i)?1:0;
			$i=$i+1;
		}
 ...
 /*代码定制*/
 /*
 根据传送的表的名字调出表中的数据得到先锋
 */
  function get_xfdfw($order=''){
 	//if(empty($tpl_id)){return;}
	if(empty($order)){$order="desc";}
	global $lang;
	//获取栏目id
	//echo "123";
	$sql="select*from ".DB_PRE."xfdfw order by id ".$order;
	$rel = $GLOBALS['mysql']->fetch_asc($sql);
	if(!empty($rel)){
		$i=1;
		$num=count($rel);
...
/*
 根据传送的表的名字调出表中的数据得到xp
 */
  function get_xptybg($id='',$order=''){
 	if(empty($id)){return;}
	if(empty($order)){$order="desc";}
	global $lang;
	//获取栏目id
	//echo "123";
	$sql="select*from ".DB_PRE."xptybg where lmid=".$id." order by id ".$order;
	$rel = $GLOBALS['mysql']->fetch_asc($sql);
	if(!empty($rel)){
		$i=1;
		$num=count($rel);
...
得带新品报告的列表页
*/
  function get_xptybg2($id='',$order='',$pgzs=''){
    global $page;
 	if(empty($id)){$id="83";}
	if(empty($order)){$order="desc";}
	$row = empty($pgzs) ? 20 : $pgzs;
	$page = empty($page) ? 1 : $page;
	$offset = ($page-1) * $row;
	global $lang;
	//获取栏目id
	//echo "123";
	$sql="select*from ".DB_PRE."xptybg where lmid=".$id." order by id ".$order." limit ".$offset.",".$pgzs;
	$path = CMS_URL;
	$rel = $GLOBALS['mysql']->fetch_asc($sql);
	if(!empty($rel)){
		$i=1;
	...
	$sql2="select count(*) as zs from ".DB_PRE."xptybg where lmid=".$id;
	$rel2=$GLOBALS['mysql']->fetch_asc($sql2);
	//print_r($rel2);
	$zs=ceil($rel2[0]['zs']/$pgzs);
	if(!empty($rel2)){
	session_start();
    $_SESSION['count']=$zs;
	}else{
...
function xptp($id=''){
        if(empty($id)){return;}
	 	$sql="select * from ".DB_PRE."xptybg where id=".$id." limit 0,1";
		//echo $sql;
		$pic_rel=$GLOBALS['mysql']->fetch_asc($sql);
		//echo "123";
		if(!empty($pic_rel)){
		//echo $pic_rel[0]['sctp'];
		$zhtp=explode("|",$pic_rel[0]['sctp']);
		//print_r($zhtp);
		$zs=count($zhtp)-1;
		$a=0;
			while($a<$zs){
 ...
 * @return  array
 */
 function get_flash($tpl_id='',$order=''){
 	if(empty($tpl_id)){return;}
	if(empty($order)){$order="desc";}
	global $lang;
	//获取栏目id
	$cate_rel=$GLOBALS['mysql']->fetch_asc("select id from ".DB_PRE."flash_ad_cate where cate_tpl_id=".$tpl_id);
	$sql="select*from ".DB_PRE."flash_ad where lang='".$lang."' and cate_id=".$cate_rel[0]['id']." order by pic_order ".$order;
	$rel = $GLOBALS['mysql']->fetch_asc($sql);
	if(!empty($rel)){
		$i=1;
		$num=count($rel);
...
function get_link_product($type=0){
	$type=empty($type)?0:$type;
	$path=CMS_URL;
	$sql="select * from ".DB_PRE."maintb where id in({$type}) and channel=3 and lang='".$GLOBALS['lang']."'";
	$rel=$GLOBALS['mysql']->fetch_asc($sql);
	$rel_arr=array();
	if(!empty($rel)){
		foreach($rel as $k=>$v){
			$rel_arr[$k]['title']=$v['title'];
			$rel_arr[$k]['thumb_pic'] = CMS_URL.'upload/'.$v['tbpic'];
			$rel_arr[$k]['id']=$v['id'];
			$url=$path.'product/show_product.php?id='.$v['id'];
			$rel_arr[$k]['url']=$url;//链接
		}
	}
	return $rel_arr;
}
 ...
 */ 
 ...
 {
 	  	$sql="select * from ".DB_PRE."brand where recommend=1";
 	  	$rel=$GLOBALS['mysql']->fetch_asc($sql);
 	  	return $rel;
 }

function getChildeCate(){
	global $id;
	$sql="select * from ".DB_PRE."category where cate_parent=".$id." order by cate_order asc";
	$cate_rel=$GLOBALS['mysql']->fetch_asc($sql);
	return $cate_rel;
}
function getChildeCateById($id){
	$sql="select * from ".DB_PRE."category where cate_parent=".$id." order by cate_order asc";
	$cate_rel=$GLOBALS['mysql']->fetch_asc($sql);
	return $cate_rel;
}
